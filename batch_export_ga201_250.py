#!/usr/bin/env python3
"""
Batch-Export von Pagebreak-Markern für GA201 bis GA250
"""
...
def main():
    ...
    # Verarbeite in Gruppen von 10 (um nicht zu viele Argumente auf einmal zu haben)
    batch_size = 10
    total_processed = 0
    total_failed = 0
    ...
    for i in range(0, len(ga_numbers), batch_size):
        ...
        if result.returncode == 0:
            ...
            print(f"\n✓ Batch {batch_num} erfolgreich verarbeitet ({len(batch)} Bände)")
        else:
            total_failed += len(batch)
            print(f"\n✗ Batch {batch_num} fehlgeschlagen")
