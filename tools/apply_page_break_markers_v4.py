...
# Hauptverzeichnis (eine Ebene über tools/)
...
def main() -> None:
    ...
    # Filtere Breaks nach contentRange (z.B. um Vorbemerkungen auszuschließen)
    content_range = ga_anchors.get("contentRange")
    if content_range and len(content_range) >= 2:
        start_page, end_page = content_range[0], content_range[1]
        original_count = len(breaks)
        breaks = [b for b in breaks if start_page <= b.get("page", 0) <= end_page]
        print(f"[V4] {ga}: Break-Anker: {len(breaks)} (contentRange [{start_page}, {end_page}], {original_count - len(breaks)} ausgeschlossen)")
    else:
