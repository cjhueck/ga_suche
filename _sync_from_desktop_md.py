# -*- coding: utf-8 -*-
"""
Synchronisiert IDs für GA205-GA209 von Desktop-Markdown-Dateien zu neuen JSON-Exporten.
"""
import json
import os
import re
...
def load_old_lectures_from_desktop(ga_number):
    ...
    # Finde alle Vortragsdateien (Pattern: GA205 (1.) ... .md)
...
def create_id_mapping(old_lectures, new_lectures, ga_number):
    ...
    for lec_id in old_lectures:
        ...
        for i, old_p in enumerate(old_paras):
            ...
            # Positionsbasiertes Matching mit Verifikation
            if i < len(new_paras):
                new_p = new_paras[i]
...
def main():
    print("=== ID-Synchronisierung von Desktop-Markdown-Dateien ===")
    ...
    for ga in TARGET_GAS:
        ...
        if not new_lectures:
            ...
            continue
        print(f"  Neue Vorträge (JSON): {len(new_lectures)}")
