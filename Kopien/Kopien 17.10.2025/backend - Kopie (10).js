...
function addRelevanceScoringToResults(results, query) {
  ...
  const resultsWithRelevance = results.map(result => {
    ...
    // Bestimme Relevanz-Kategorie (Schwellen weiter gesenkt für bessere Verteilung)
...
app.get('/api/ga-list', async (req, res) => {
  try {
    const gaNumbers = Object.keys(fullLectures)
      .map(lectureId => {
        const parts = lectureId.split('/');
        return parts[0]; // GA-Nummer
      })
      .filter((gaNum, index, array) => array.indexOf(gaNum) === index) // Deduplizieren
      .sort((a, b) => {
        // Sortiere numerisch: GA001, GA002, ..., GA010, GA011, etc.
...
- **KRITISCH WICHTIG**: Füge den Absatz-Index in runden Klammern nach jedem Zitat hinzu (z.B. (GA087/18:5))
- **JEDES Zitat MUSS einen Index haben** - ohne Index ist das Zitat unvollständig
...
- Beispiel: "Die anschauende Urteilskraft ist... (GA087/18:5)" statt "Goethes Auseinandersetzung mit Kant..."
