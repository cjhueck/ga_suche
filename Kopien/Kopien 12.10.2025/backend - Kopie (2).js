...
app.post('/api/keywords', async (req, res) => {
  try {
    const { keyword, summary = '', matches = [], confidence = 0 } = req.body;
    ...
    db[key].matches = matches;
    db[key].confidence = confidence;
    db[key].timestamp = new Date().toISOString();

    const ok = await saveKeywordsDatabase(db);
    if (ok) res.json({ success: true, keyword: key });
