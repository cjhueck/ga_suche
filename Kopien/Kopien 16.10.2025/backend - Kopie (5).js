...
async function generateKeywordTimelineAnalysis(keyword, allLectures) {
  ...
  // Erstelle Kontext-Text für KI-Analyse - alle Vorträge, aber mit Text-Begrenzung
  const contextText = relevantLectures
    .map(lecture => {
      ...
      // Begrenze die Textlänge pro Absatz (max. 500 Zeichen)
      const limitedParagraphs = relevantParagraphs.map(p => ({
        ...p,
        content: p.content.length > 500 ? p.content.substring(0, 500) + '...' : p.content
      }));
      
      // Begrenze die Anzahl der relevanten Absätze pro Vortrag (max. 5)
      const finalParagraphs = limitedParagraphs.slice(0, 5);
      
      const paragraphsText = finalParagraphs
