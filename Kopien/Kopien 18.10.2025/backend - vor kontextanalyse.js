...
// Hilfsfunktion: Relevanz-Score für einen Vortrag berechnen (1000-Wörter-Fenster Version)
function calculateRelevanceScoreForLecture(lectureResults, query) {
  ...
  // 3. Normalisierung mit Gesamtvorkommen
  const totalOccurrenceFactor = Math.sqrt(totalOccurrences);
  
  // Finaler Score - stark reduzierte Skalierung für bessere Differenzierung
  const finalScore = bestWindowScore * totalOccurrenceFactor * 5; // Von 100 auf 5 reduziert
  
  // Debug-Ausgabe
  if (totalOccurrences > 0) {
    console.log(`[RELEVANCE-1000W] Query="${query}", TotalOcc=${totalOccurrences}, TotalWords=${totalWords}, BestWindow=${bestWindowScore.toFixed(6)}, TotalFactor=${totalOccurrenceFactor.toFixed(2)}, FinalScore=${Math.min(finalScore, 1).toFixed(3)}`);
  }
